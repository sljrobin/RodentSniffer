...
/* Find and printing all the available devices */
void rosn_boot_devs_list(void)
{
    ...
    char devslst[ROSN_MAX_DEVICES][ROSN_MAX_DEVICES] = {{0}};
    ...
    rosn_clrset ptstep_info; rosn_utils_clrprnt_setattr(rosn_utils_clrprnt_setfg(rosn_utils_clrprnt_init(&ptstep_info), ROSN_CLR_WHITE), ROSN_ATTR_NONE);                /* Step info */
    rosn_clrset ptstep_inprogress; rosn_utils_clrprnt_setattr(rosn_utils_clrprnt_setfg(rosn_utils_clrprnt_init(&ptstep_inprogress), ROSN_CLR_BLUE), ROSN_ATTR_NONE);  /* Step in progress */
